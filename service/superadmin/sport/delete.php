...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
    ...
    $stmt = $conn->prepare("SELECT `image` FROM `sport` WHERE `id` = :id");
    ...
    if ($result) {
        ...
        // ลบรูปภาพเดิม (ถ้ามี)
        if (!empty($imageToDelete) && file_exists('../../uploadsport/' . $imageToDelete)) {
            unlink('../../uploadsport/' . $imageToDelete);
        }

        // ลบข้อมูลจากฐานข้อมูล
        $deleteStmt = $conn->prepare("DELETE FROM `sport` WHERE `id` = :id");
